...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        H = W = int(np.sqrt(self.input_size))
...
class CVAE(nn.Module):
    ...
    def forward(self, x, c):
        ...
        # first flatten the images for concatenation
        x = torch.concat([torch.flatten(x, 1, -1), c], dim=1)
...
def reparametrize(mu, logvar):
    ...
    std_z = torch.normal(torch.zeros_like(mu), torch.ones_like(logvar))
    z = std_z * torch.sqrt(torch.exp(logvar)) + mu
...
def loss_function(x_hat, x, mu, logvar):
    ...
    KL_loss = - 0.5 * torch.sum(torch.ones_like(mu) + logvar - mu**2 - torch.exp(logvar))
    loss = (reconstruction_loss + KL_loss) / x.size(0)
