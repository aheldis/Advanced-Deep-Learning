...
def sample_noise(batch_size, dim, seed=None):
    ...
    noise = torch.rand(batch_size, dim)     # [0, 1)
    return 2 * noise - 1
...
def discriminator(seed=None):
    ...
    # default: alpha=1e-2, Linear includes bias
...
def discriminator_loss(logits_real, logits_fake):
    ...
    labels_real = torch.ones(logits_real.size(0)).type(dtype)
    labels_fake = torch.zeros(logits_fake.size(0)).type(dtype)
...
def generator_loss(logits_fake):
    ...
    # we use all 1's as the label to compute the generator loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    mse = nn.MSELoss()
    loss_real = mse(scores_real, torch.ones(scores_real.size()).type(dtype))
    loss_fake = mse(scores_fake, torch.zeros(scores_fake.size()).type(dtype))
...
def ls_generator_loss(scores_fake):
    ...
    mse = nn.MSELoss()
    loss = 0.5 * mse(scores_fake, torch.ones(scores_fake.size()).type(dtype))
